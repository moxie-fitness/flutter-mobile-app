...
class Carousel extends StatefulWidget {
  //All the widgets on this Carousel.
  final List widgets;
  ...
  //How to show the widgets in the box. Default is cover
  final BoxFit boxFit;

  //Enable/Disable radius Border for the widgets. Default is false
  final bool borderRadius;

  //Border Radius of the widgets. Default is [Radius.circular(8.0)]
  final Radius radius;
  ...
  Carousel({
    ...
    this.radius,
    this.moveIndicatorFromBottom = 5.0,
...
class CarouselState extends State<Carousel> {
  ...
  void initState() {
    ...
    if (widget.autoplay) {
      new Timer.periodic(widget.autoplayDuration, (_) {
        if (_controller.page == widget.widgets.length - 1) {
  ...
  Widget build(BuildContext context) {
    final List<Widget> listWidgets = widget.widgets
        .map((widgetItem) => new Stack(
              children: <Widget>[
                widgetItem,
                widget.overlayShadow
                    ? new GestureDetector(
...
//                color: widget.dotBgColor == null ? Colors.grey[800].withOpacity(0.5) : widget.dotBgColor,
                    ...
                    padding:
                        new EdgeInsets.only(top: widget.indicatorBgPadding),
                    child: new Center(
